...
$app->match('/assets/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['employer_id'], $data['asset_name'], $data['description'], 1490204423, 1490204423));            
...
$app->match('/assets/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['employer_id'], $data['asset_name'], $data['description'], $data['created_at'], 1490204423, $id));            
