...
$app->match('/article/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['slug'], $data['title'], $data['body'], $data['view'], $data['category_id'], $data['thumbnail_base_url'], $data['thumbnail_path'], $data['status'], $data['created_by'], $data['updated_by'], $data['published_at'], 1490680861, 1490680861));            
...
$app->match('/article/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['slug'], $data['title'], $data['body'], $data['view'], $data['category_id'], $data['thumbnail_base_url'], $data['thumbnail_path'], $data['status'], $data['created_by'], $data['updated_by'], $data['published_at'], $data['created_at'], 1490680861, $id));            
