...
$app->match('/city/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['name'], $data['state_id'], $data['country_id'], 1490680861, 1490680861));            
...
$app->match('/city/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['name'], $data['state_id'], $data['country_id'], $data['created_at'], 1490680861, $id));            
