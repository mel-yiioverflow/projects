...
$app->match('/city/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "INSERT INTO `city` (`state_id`, `name`, `created_at`, `updated_at`) VALUES (?, ?, ?, ?)";
            $app['db']->executeUpdate($update_query, array($data['state_id'], $data['name'], 1490611206, 1490611206));            
...
$app->match('/city/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $update_query = "UPDATE `city` SET `state_id` = ?, `name` = ?, `created_at` = ?, `updated_at` = ? WHERE `id` = ?";
            $app['db']->executeUpdate($update_query, array($data['state_id'], $data['name'], $data['created_at'], 1490611206, $id));            
