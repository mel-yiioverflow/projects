...
$app->match('/contacts/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['employer_id'], $data['name'], $data['description'], $data['phone'], $data['email'], $data['location'], $data['city_id'], $data['state_id'], $data['country_id'], 1490178417, 1490178417));            
...
$app->match('/contacts/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['employer_id'], $data['name'], $data['description'], $data['phone'], $data['email'], $data['location'], $data['city_id'], $data['state_id'], $data['country_id'], $data['created_at'], 1490178417, $id));            
