...
$app->match('/user/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['username'], $data['auth_key'], $data['access_token'], $data['password_hash'], $data['oauth_client'], $data['oauth_client_user_id'], $data['email'], $data['status'], 1490680861, 1490680861, $data['logged_at']));            
...
$app->match('/user/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['username'], $data['auth_key'], $data['access_token'], $data['password_hash'], $data['oauth_client'], $data['oauth_client_user_id'], $data['email'], $data['status'], $data['created_at'], 1490680861, $data['logged_at'], $id));            
