...
$app->match('/page/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['slug'], $data['title'], $data['body'], $data['view'], $data['status'], 1490680861, 1490680861));            
...
$app->match('/page/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['slug'], $data['title'], $data['body'], $data['view'], $data['status'], $data['created_at'], 1490680861, $id));            
