...
$app->match('/rbac_auth_item/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['name'], $data['type'], $data['description'], $data['rule_name'], $data['data'], 1490178417, 1490178417));            
...
$app->match('/rbac_auth_item/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['name'], $data['type'], $data['description'], $data['rule_name'], $data['data'], $data['created_at'], 1490178417, $id));            
