...
$app->match('/key_storage_item/create', function () use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['key'], $data['value'], $data['comment'], 1490680861, 1490680861));            
...
$app->match('/key_storage_item/edit/{id}', function ($id) use ($app) {
    ...
    if("POST" == $app['request']->getMethod()){
        ...
        if ($form->isValid()) {
            ...
            $app['db']->executeUpdate($update_query, array($data['key'], $data['value'], $data['comment'], 1490680861, $data['created_at'], $id));            
